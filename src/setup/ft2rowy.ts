...
export const checkIfFTMigrationRequired = async (req:Request,res:Response) => {
  const ftSettingsExists = (await db.doc(FT_SETTINGS).get()).exists;
  if (!ftSettingsExists) return res.send({migrationRequired: false,message:"Firetable Settings doesn't exist"});
  const migrated =
    (await db.doc(ROWY_SETTINGS).get()).data()?.migratedToV2 !== undefined;
  if (migrated) return res.send({migrationRequired: false,message:"Firetable Settings has already been migrated"});
  const tableSchemas = (
    await db.collection(FT_TABLE_SCHEMAS).get()
  ).size;

  const groupTableSchemas = (
    await db.collection(FT_GROUP_TABLE_SCHEMAS).get()
  ).size;
  if(tableSchemas + groupTableSchemas === 0)return res.send({migrationRequired: false,message:"Firetable Settings has no tables"});
  return res.send({migrationRequired: true,message:"Firetable Settings needs to be migrated"});
}



export const migrateFT2Rowy = async () => {
  const migrated = (await db.doc(ROWY_SETTINGS).get()).data()?.migratedToV2 !== undefined;
  ...
  await db
    .doc(ROWY_SETTINGS)
    .set({ table: newTables}, { merge: true });
  ...
  ),...groupTables.docs.map((table) =>db.collection(ROWY_GROUP_TABLE_SCHEMAS).doc(table.id).set(table.data(), { merge: true }))]
