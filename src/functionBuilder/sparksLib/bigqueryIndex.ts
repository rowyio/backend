...
const bigqueryIndex = async (payload, sparkContext) => {
  ...
  const tableFullName = `${_projectID}.rowy.${index}`;
  console.log(
    `projectID: ${_projectID}, index: ${index}, tableFullName: ${tableFullName}`
  );

  // create dataset with exact name "rowy" if not exists
  async function preprocessDataset() {
    const dataset = bigquery.dataset("rowy", {
      location: datasetLocation ?? "US",
    });
    const res = await dataset.exists();
    const exists = res[0];
    if (!exists) {
      console.log("Dataset 'rowy' does not exist, creating dataset...");
      await dataset.create();
      console.log("Dataset 'rowy' created.");
    } else {
      console.log("Dataset 'rowy' exists.");
    }
  }
  ...
  async function preprocessTable() {
    ...
    if (!exists) {
      console.log(
        `Table '${index}' does not exist in dataset 'rowy', creating dataset...`
      );
      await table.create();
      console.log(`Table '${index}' created in dataset 'rowy'.`);
    } else {
      console.log(`Table ${index} exists in 'rowy'.`);
    }
  }
  ...
  function getTypeKnownRecord(data) {
    ...
    if (unknownKeys?.length > 0) {
      console.log(
        "The following fields do not exist in Rowy and are ignored.",
        unknownKeys
      );
    }
